    ...
    >
      <div className="max-w-5xl mx-auto">
        <h2 className="text-2xl md:text-3xl font-bold mb-12 flex items-center gap-3">
          <span className="text-brand-purple font-mono text-xl">04.</span>
          Habilidades & Idiomas
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {skillCategories.map((cat, index) => {
            ...
            return (
              <div
                key={index}
                className="bg-dark-card border border-dark-border p-6 rounded-xl"
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {cat.skills.map((skill, idx) => (
                    <span
                      key={idx}
                      className="bg-dark-bg text-slate-300 text-xs px-3 py-1 rounded-md border border-dark-border/60"
                    >
                      {skill}
                    </span>
                  ))}
